...
const DICT = {
  en: {
    ...
    price_sar: "Price (SAR)",
    category: "Category",
    unsplash_image_id: "Unsplash Image ID",
    apparel: "Apparel",
    footwear: "Footwear", 
  ...
  ar: {
    ...
    price_sar: "السعر (ر.س)",
    category: "الفئة",
    unsplash_image_id: "معرف صورة Unsplash",
    apparel: "الملابس",
    footwear: "الأحذية",
...
function renderCatalogNew(){
  ...
  v.innerHTML = html(`
    <section class="panel">
      ...
      <label>${t("product_name")}<input id="p_name" placeholder="Product name"></label>
      <label>${t("price_sar")}<input id="p_price" type="number" value="99"></label>
      <label>${t("category")}<select id="p_cat"><option>${t("apparel")}</option><option>${t("footwear")}</option><option>${t("accessories")}</option><option>${t("beauty")}</option></select></label>
      <label>${t("unsplash_image_id")}<input id="p_img" placeholder="1519744792095-2f2205e87b6f"></label>
        ...
        <div style="background:var(--bg2); border-radius:12px; padding:20px; border:1px solid var(--border)">
          ...
          ${upcomingStreams.map(stream => `
            <div class="row between" style="padding:12px; background:var(--bg); border-radius:8px; margin-bottom:8px">
              ...
              <button class="small ghost">${t("edit")}</button>
            </div>
          `).join('')}
        </div>
      ...
      ${live ? `
        ...
        <div style="background:var(--bg2); border-radius:8px; padding:16px; margin-top:16px">
          ...
          <div class="grid cols-2" style="gap:16px">
            <div>
              ...
              <div class="row between" style="margin-bottom:8px">
                <span class="muted small">${t("uae")}</span>
                <span class="small">12%</span>
              </div>
              <div class="row between">
                <span class="muted small">${t("other")}</span>
        ...
        <div class="panel" style="margin-top:16px; background:var(--card)">
          ...
          <div class="row" style="gap:8px; margin-top:8px">
            ...
            <button class="secondary small" onclick="spotlightProduct()">${t("spotlight")}</button>
            <button class="ghost small" onclick="addFlashDeal()">⚡ ${t("flash_deal")}</button>
          </div>
          ${featuredProduct ? `
        ...
        <div style="margin-top:16px">
          ...
          <div class="panel" style="margin-top:16px; background:var(--card)">
            <div class="row between" style="margin-bottom:12px">
              ...
              <button class="small secondary" onclick="navigate('#/creator')">${t("manage")}</button>
            </div>
...
function renderInbox(){
  const rows = state.inbox.map(ti=>`
    <div class="item">
      ...
      <a class="chip" href="javascript:void(0)" onclick="alert('Open thread ${ti.id}')">${t("open")}</a>
    </div>
  `).join("");
  qs("#view").innerHTML = html(`
    <section class="panel">
    ...
    <section class="panel">
      ...
      <div class="kpis" style="margin-top:16px">
        ...
        <div class="kpi" style="cursor:pointer" onclick="navigate('#/creator')" title="View engagement details">
          <div class="head">${t("engagement")}</div>
  ...
  v.innerHTML = html(`
    <section class="panel">
      ...
      <div class="kpis" style="margin-top:10px">
        <div class="kpi"><div class="head">${t("ctr")}</div><div class="val">${(Math.random()*4+2).toFixed(1)}%</div></div>
        <div class="kpi"><div class="head">${t("cvr")}</div><div class="val">${(Math.random()*3+1).toFixed(1)}%</div></div>
        <div class="kpi"><div class="head">${t("aov")}</div><div class="val">${fmtSAR(m.aov)}</div></div>
        <div class="kpi"><div class="head">${t("gmv")}</div><div class="val">${fmtSAR(m.gmv30)}</div></div>
      </div>
  ...
  if (post) {
    ...
    setSheet(`Post Details`, `
      <div style="max-width:400px">
        ...
        ${post.status === 'pending' ? `
          <div class="row" style="gap:8px">
            <button class="secondary" onclick="moderatePost('${post.id}', 'approved'); window.__closeSheet()">${t("approve")}</button>
            <button class="ghost" onclick="moderatePost('${post.id}', 'rejected'); window.__closeSheet()">${t("reject_post")}</button>
          </div>
        ` : ''}
      </div>
    `);
